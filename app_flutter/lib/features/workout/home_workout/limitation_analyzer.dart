...
class HomeLimitationAnalyzer {
  ...
  static HomeExerciseAnalysis analyze({
    ...
    return HomeExerciseAnalysis(
      ...
      reason:
          'Demanda do exercício não é apropriada para a capacidade ou resposta atual.',
    );
  }
  ...
  static HomeExercise? _findRegression(HomeExercise exercise) {
    ...
    for (final candidate in homeExerciseLibrary) {
      if (candidate.id == exercise.regressionId) return candidate;
    }
    return null;
  }
  ...
  static HomeExercise? _findAlternative(HomeExercise exercise) {
    ...
    for (final alternativeId in exercise.alternativeIds) {
      for (final candidate in homeExerciseLibrary) {
        if (candidate.id == alternativeId) return candidate;
      }
    }
    return null;
  }
    ...
    switch (feedback.quality) {
      case HomeFeedbackQuality.executedWell:
        return HomeFeedbackAnalysis(
          ...
          suggestedExercise: exercise.progressionId != null
              ? 'progression'
              : null,
        );
      ...
      case HomeFeedbackQuality.executedWithAdaptation:
        return HomeFeedbackAnalysis(
          ...
          reason:
              'Executou com adaptação. Manter adaptação ou testar pequena progressão.',
      ...
      case HomeFeedbackQuality.worsenedSymptoms:
        return HomeFeedbackAnalysis(
          ...
          reason:
              'Piorou os sintomas. Evitar esta variação e procurar alternativa.',
